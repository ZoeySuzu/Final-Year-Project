...
public class PlayerController2 : MonoBehaviour {
    ...
    public static PlayerController2 Instance { get; set; }
    private void Awake()
    {
	...
	void Update () {
        ...
        if (playerState.Contains("Climbing"))
        {
            ...
            if (z != 0)
            {
                transform.Translate(Vector3.up *z*3*Time.deltaTime);
            }
        }
        //Pushing
        ...
        else
        {
            ...
            {
                lockRotation = true;
                Vector3 orientation = new Vector3(focus.transform.position.x, transform.position.y, focus.transform.position.z);
    ...
    private void Shield()
    {
        ...
        {
            var activeShield = Instantiate(shield, transform.position, transform.rotation);
        }
    }
    ...
    private void Aim()
    {
        ...
        else if (Input.GetAxis("R2") > 0.5f && FollowCamera.Instance.aim)
        {
            ...
            //if(!FollowCamera.Instance.getLerp())
                transform.forward = new Vector3(FollowCamera.Instance.transform.forward.x, 0, FollowCamera.Instance.transform.forward.z);
        }
    ...
    private void SpellCast()
    {
        if (((Input.GetAxis("R2")>0.5f) && (FollowCamera.Instance.getFocus()!= null) || (playerState.Contains("Aiming") && Input.GetButtonDown("X"))) && !spellCasting && spellCooldown.ready)
        {
            ...
            spell = Instantiate(projectile, transform.position + transform.forward * 2 + Vector3.up * 0.5f, transform.rotation).GetComponent<Spell>();
    ...
    private void TrapCast()
    {
        if (grounded && Input.GetButtonDown("Y"))
        {
            ...
            if (spellTrap[i] == null)
            {
                spellTrap[i] = Instantiate(trap, transform.position - new Vector3(0, transform.localScale.y, 0), transform.rotation).GetComponent<Trap>();
    ...
    private void Attack()
    {
        ...
        {
            canAttack = false;
            Instantiate(attackCollider, transform.position + transform.forward * 1.5f + transform.up * 0.5f + transform.right * 1f, transform.rotation).transform.parent = transform;
    ...
    private bool IsGrounded() {
        if (Physics.CapsuleCast(transform.position, transform.position + transform.up, 0.5f, Vector3.down, 0.6f, -1, QueryTriggerInteraction.Ignore))
        {
    ...
    IEnumerator Strafe(Vector3 direction)
    {
        ...
        for (int i = 1; i <= 30; i++)
        {
            ...
            {
                Movement(transform.right * x, speed* 2.5f);
            }
    ...
    private void DoubleJump()
    {
        ...
        rb.AddForce(transform.transform.forward * -100);
    }
    ...
    private void Movement(Vector3 targetDirection, float _speed)
    {
        if (!FollowCamera.Instance.lerping)
            transform.forward = targetDirection;
        RaycastHit hit;

        if (Physics.CapsuleCast(transform.position, transform.position+transform.up*1f, 0.5f,transform.forward*targetDirection.magnitude, out hit, 0.2f))
        {
            transform.position += transform.forward * hit.distance* 0.99f*_speed*Time.deltaTime;
        }
        else
        {
            transform.position += transform.forward * targetDirection.magnitude * _speed * Time.deltaTime;
        }
    }
